    ...
    import   JPTfns from '../JPTs/getAPI_u1.02.mjs'
...
//  var aCREDs     = "credentials": "same-origin"
    ...
    var pData      =  { "message": "What is AnythingLLM?", "mode": "query" }

      console.log( `  ${aMethod}:     ${aURL}` )
      console.log( `  pData:    ${ inspect( pData, { depth: 9 } ) }` )
      console.log( `  aAPI_KEY: ${aAPI_KEY}` )

//  var pResponse  =  await getAPI( aURL, aMethod, pData, aAPI_KEY )

    var pResponse  =  await JPTfns.getAPI( aURL, pData, aANYLLM_API_KEY )
